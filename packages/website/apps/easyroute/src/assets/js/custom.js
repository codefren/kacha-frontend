...
*/
        ...
        $(function() {
            $('.card')
                .on('card:toggle', function() {
                    ...
                    $this
                        .find('.card-body, .card-footer')['slide' + direction](200, function() {
                            $this[(direction === 'Up' ? 'add' : 'remove') + 'Class'](
                                'card-collapsed',
                            );
                        });
                })
                .on('card:dismiss', function() {
                    ...
                    if (!!($this.parent('div').attr('class') || '').match(
                            /col-(xs|sm|md|lg)/g,
                        ) &&
    ...
    (function(theme, $) {
        ...
        PluginDatePicker.prototype = {
            ...
            build: function() {
                ...
                if (!!this.skin &&
                    typeof this.$el.data('datepicker').picker != 'undefined'
                ) {
    ...
    (function(theme, $) {
        ...
        GMapBuilder.prototype = {
            ...
            _appendMarkerToList: function(marker) {
                ...
                html = html.replace(
                    /\{location\}/, !!marker.title ? marker.title : marker.location,
                );
            ...
            _bindMarkerClick: function(marker) {
                ...
                html = html.replace(
                    /\{title\}/, !!marker.title ? '<h4>' + marker.title + '</h4>' : '',
                );
                html = html.replace(
                    /\{description\}/, !!marker.description ? '<p>' + marker.description + '</p>' : '',
                );
    ...
    (function(theme, $) {
        ...
        PluginMultiSelect.defaults = {
            templates: {
                ...
                filter: '<div class="input-group"><span class="input-group-prepend"><span class="input-group-text"><img class="icons-search" src="assets/icons/optimmanage2/icono-lupanaranja.svg"></span></span><input class="form-control multiselect-search" type="text"></div>',
            },
        };
                ...
                if (!!data) {
                    $.each(data, function(panelId, state) {
                        ...
                        if (!panel.data('portlet-state-loaded')) {
                            if (state == 'collapsed') {
                                panel
                                    .find('.card-actions a.fa-caret-down')
                                    .trigger('click');
    ...
    (function(theme, $) {
        ...
        var Skeleton = {
            ...
            buildSidebarLeft: function() {
                ...
                this.sidebars.left.isOpened = !$html.hasClass('sidebar-left-collapsed') ||
            ...
            eventsContentMenu: function() {
                ...
                var close = function(e) {
                    ...
                    isCollapseButton =
                        hasTarget && !!$target.closest('.inner-menu-collapse').get(0);
                    ...
                    if (
                        (!isCollapseButton && (isInsideInnerMenu || !isInsideHTML)) ||
                        isInsideModal
                    ) {
                        return false;
                    }
    ...
    (function($) {
        ...
        if ($.isFunction($.fn['easyPieChart'])) {
            $(function() {
                $('[data-plugin-chart-circular], .circular-bar-chart:not(.manual)').each(
                    function() {
                        var $this = $(this),
                            opts = {};
            ...
            select: function(idx) {
                ...
                $(segment.seg[0]).hover(
                    function() {
                        ...
                        if (row && row.measure && row.measure.length > 0) {
                            row.measure.forEach(element => {
                                li = li + `${'<li>'+( element.measure && element.measure !== null ? 
                                (element.totalQuantity + ' '  + element.measure) : (element.totalQuantity + ' No especificado')) 
                                +'</li>'}`
                            });
                        }
                        ...
                        const errorMsg = `${'<h4>'+ row.label +'</h4>'}`;
                        const errorElem = document.createElement('div');
                        errorElem.innerHTML = errorMsg;
                        /* errorElem.innerHTML = li; */
                        errorElem.style.position = "absolute";
                        errorElem.style.top = `${(data && data.top) ? data.top - 50: '0'}px`;
                        errorElem.style.left = `${(data && data.left) ? data.left : '0'}px`;
                        errorElem.zIndex = 900;
                        errorElem.id = idx;
                        errorElem.classList.add("message-total-client");
                ...
                this.fire('hover', idx, this.data[idx]);
                return this.setLabels(row.label, this.options.formatter(row.value, row));
            },
        };

        $.extend(true, Morris.Donut.prototype, select);
