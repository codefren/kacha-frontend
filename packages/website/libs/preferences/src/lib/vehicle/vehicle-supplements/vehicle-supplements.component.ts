...
import { ModalAddSupplementsComponent } from './modal-add-supplements/modal-add-supplements.component';
import { AuthLocalService } from '@optimroute/auth-local';
import { LoadingService, ToastService } from '@optimroute/shared';
import { BackendService } from '@optimroute/backend';

@Component({
  selector: 'easyroute-vehicle-supplements',
  templateUrl: './vehicle-supplements.component.html',
  styleUrls: ['./vehicle-supplements.component.scss']
})
export class VehicleSupplementsComponent implements OnInit {
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_supplement_datatables';
    ...
    this.initEvents('#supplements tbody', this.table);
}
  ...
  load(){
    ...
    this.backendService.get('company_supplement').pipe(take(1)).subscribe((data)=>{
  ...
  updateIsActive(Id: number, element: any , vehicle: any) {
    ...
    this.backendService.put('company_supplement/'+Id,{isActive:element}).pipe(take(1)).subscribe((data)=>{
  ...
  modalAddOtherLicense(){
    ...
    const modal = this._modalService.open(ModalAddSupplementsComponent, {
  ...
  deleteOtherLicense(data:any){
    ...
    modal.componentInstance.message = this.translate.instant('VEHICLES.SETTINGS.MESSAGE_DELETE_SUPPLEMENTS');
