...
import { ModalAddAccesoriesComponent } from './modal-add-accesories/modal-add-accesories.component';
...
@Component({
  selector: 'easyroute-vehicle-digital-accessories',
  templateUrl: './vehicle-digital-accessories.component.html',
  styleUrls: ['./vehicle-digital-accessories.component.scss']
})
export class VehicleDigitalAccessoriesComponent implements OnInit {
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_digital_complement_datatables';
    let tok = 'Bearer ' + this.authLocal.obtenerTokenLocalStorage();
    let table = '#OtherLicenses';
    ...
    this.initEvents('#OtherLicenses tbody', this.table);
}
  ...
  load(){
    ...
    this.backendService.get('company_digital_complement').pipe(take(1)).subscribe((data)=>{
...
updateIsActive(Id: number, element: any , vehicle: any) {
  ...
  this.backendService.put('company_digital_complement/'+Id,{isActive:element}).pipe(take(1)).subscribe((data)=>{
...
modalAddDigitalAccesories(){

    const modal = this._modalService.open(ModalAddAccesoriesComponent, {
  ...
  deleteOtherLicense(data:any){
    ...
    modal.componentInstance.message = this.translate.instant('VEHICLES.SETTINGS.MESSAGE_DELETE_DIGITAL_ACCESORIES');
