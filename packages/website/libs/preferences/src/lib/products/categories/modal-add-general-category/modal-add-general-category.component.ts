...
import { ProductModalImgInfoComponent } from '@optimroute/shared';
import * as _ from 'lodash';
declare var $: any;
declare function init_plugins();
@Component({
  selector: 'easyroute-modal-add-general-category',
  templateUrl: './modal-add-general-category.component.html',
  styleUrls: ['./modal-add-general-category.component.scss']
})
export class ModalAddGeneralCategoryComponent implements OnInit {
 ...
 load(){
     ...
     if ( this.id !== 'new' ) {
       ...
       this.stateEasyrouteService.getCategory(this.id).subscribe(
           (data: any) => {
           ...
           },
           {
            ...
            render: (data, type, row) => {
              let botones = '<div class="text-center">';
                  ...
                  botones +=
                  '<button class="btn delete" title="Eliminar subcategoria vinculada"><img src="assets/icons/delete.svg"></button>';
...
openFormEditSubcategory(filterId: number, element: any, subCategory: any) {
 ...
 setImagesArray( images: Array<{ id: number, urlimage: string, image: string, list: boolean }> ) {
  ...
  create(){
    console.log('CREAR O EDITAR EL FORMLULARIO');
  }
}
