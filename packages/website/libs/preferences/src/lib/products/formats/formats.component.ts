...
import { ModalAddFormatsSettingsComponent } from './modal-add-formats-settings/modal-add-formats-settings.component';
...
export class FormatsComponent implements OnInit, OnDestroy {
  ...
  cargar() {
    ...
    if (this.tableMeasure) {

        this.tableMeasure.clear();

        this.tableMeasure.state.clear();
    }
    ...
    let table = '#measureSettings';

    this.tableMeasure = $(table).DataTable({
        destroy: true,
        serverSide: true,
        processing: true,
        stateSave: false,
       ...
       columns: [
        ...
        },
        {
          ...
          render: ( data, type, row ) => {
            ...
            if ( data ) {
              return (`
                <div class="justify-content-start row reset">
                  <span class="pl-3 style-general-table-colummns">Si</span>
                </div>
              `);
            }
            ...
            return (`
              <div class="justify-content-start row reset">
                  <span class="pl-3 style-general-table-colummns"> No</span>
              </div> 
            `);
    ...
    $('.dataTables_filter').removeAttr('class');
        this.editar('#measureSettings tbody', this.tableMeasure);
        this.isActive('#measureSettings tbody', this.tableMeasure);
...
activateMeasure(productId: number, element: any, product: any) {
  ...
  const modal = this._modalService.open(MeasureModalActiveComponent, {
      backdrop: 'static',
...
editActiveMeasure(productId: number, element: any) {
  ...
  this.backendService.put('measure/' + productId, element).pipe(take(1)).subscribe(
...
openModalCreateFormat(){

    const modal = this._modalService.open( ModalAddFormatsSettingsComponent, {
        backdrop: 'static',
        backdropClass: 'customBackdrop',
        size :'lg',
        centered: true,
    ...
    modal.componentInstance.title = this._translate.instant('CONFIGURATIONS.FORMAT.FORMATS');
