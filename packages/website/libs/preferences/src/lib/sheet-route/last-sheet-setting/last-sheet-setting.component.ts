...
@Component({
  selector: 'easyroute-last-sheet-setting',
  templateUrl: './last-sheet-setting.component.html',
  styleUrls: ['./last-sheet-setting.component.scss']
})
export class LastSheetSettingComponent implements OnInit {
  ...
  ngOnChanges() {
   ...
   filterData(data: any){
     ...
     let datas =data.find((x: any) => x.id === 2);
 
     this.tableLasted = datas;
   ...
   updateStateActive(Event: any, FormId: number, i: any){
    ...
    this.showForm[i].isActive = Event;
     ...
     let sent ={
         ...
         routeSheetTypeId: this.tableLasted.id,
