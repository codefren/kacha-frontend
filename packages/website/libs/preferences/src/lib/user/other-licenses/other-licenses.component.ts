...
import { ModalOtherLicensesComponent } from './modal-other-licenses/modal-other-licenses.component';
...
@Component({
  selector: 'easyroute-other-licenses',
  templateUrl: './other-licenses.component.html',
  styleUrls: ['./other-licenses.component.scss']
})
export class OtherLicensesComponent implements OnInit {
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_other_license_datatables';
    ...
    this.table = $(table).DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                title: this.translate.instant('USERS.SETTINGS.TYPE_LICENSE'),
                render: (data, type, row) => {
                    if (data == null || data == 0) {
  ...
  load(){
    ...
    this.backendService.get('company_other_license').pipe(take(1)).subscribe((data)=>{
  ...
  updateIsActive(Id: number, element: any , vehicle: any) {
    ...
    this.backendService.put('company_other_license/'+Id,{isActive:element}).pipe(take(1)).subscribe((data)=>{
  ...
  modalAddOtherLicense(){
    ...
    const modal = this._modalService.open(ModalOtherLicensesComponent, {
  ...
  deleteOtherLicense(data:any){
    ...
    modal.componentInstance.message = this.translate.instant('USERS.SETTINGS.MESSAGE_DELETE_COMPLEMENTARY_PERMITS');
