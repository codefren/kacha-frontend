...
import { ModalAddTypePackageComponent } from './modal-add-type-package/modal-add-type-package.component';
...
@Component({
  selector: 'easyroute-type-package-setting',
  templateUrl: './type-package-setting.component.html',
  styleUrls: ['./type-package-setting.component.scss']
})
export class TypePackageSettingComponent implements OnInit {

  typePackageList: any [] =[];

  tableTypePackage: any;
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_package_type_datatables';
    let tok = 'Bearer ' + this.authLocal.obtenerTokenLocalStorage();
    let table = '#typePackage';

    this.tableTypePackage = $(table).DataTable({
        ...
        columns: [
            ...
            },
            {
                data: 'name',
                className: 'text-aling-table',
                title: this.translate.instant('LOADING_DOCK.SETTING.TYPE_PACKAGE'),
    ...
    this.initEvents('#typePackage tbody', this.tableTypePackage);
}
  ...
  editar(tbody: any, table: any, that = this) {
    $(tbody).on('click', 'span.editar', function() {
        ...
        that.modalEditTypePackage(data);
    });
  }
  ...
  updateIsActive(Id: number, element: any , typePackage: any) {

    this.backendService.put('company_package_type/'+Id,{isActive:element}).pipe(take(1)).subscribe((data)=>{

      this.tableTypePackage.ajax.reload();
  ...
  modalAddTypePackage(){
    ...
    const modal = this._modalService.open(ModalAddTypePackageComponent, {
    ...
    modal.result.then(
      (data) => {
        if (data) {
          ...
          console.log(data,'data si es crear ');
  ...
  deleteTypePackage(data:any){
    ...
    modal.componentInstance.message = this.translate.instant('LOADING_DOCK.SETTING.MESSAGE_DELETE_TYPE_PACKAGE');
