...
declare var $: any;
@Component({
  selector: 'easyroute-route-integration',
  templateUrl: './route-integration.component.html',
  styleUrls: ['./route-integration.component.scss']
})
export class RouteIntegrationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.optimizationPreferences$.pipe(takeUntil(this.unsubscribe$)).subscribe((op) => {
        ...
        console.log(op.activateRouteIntegration, 'respuesta del servicios');
  
        if (op.activateRouteIntegration) {

            this.toggleIntegration =  op.activateRouteIntegration
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_preference_integration_route_datatables';

    let tok = 'Bearer ' + this.authLocal.obtenerTokenLocalStorage();

    let table = '#routeIntegration';
    ...
    this.editar('#routeIntegration tbody', this.tableDriverHaveServedClientTable);
    this.trash('#routeIntegration tbody', this.tableDriverHaveServedClientTable);
...
deleteSpesification(vehicleId: number) {
    ...
    this.backendService.delete('company_preference_integration_route/' + vehicleId).pipe(take(1)).subscribe( (data: any) => {
