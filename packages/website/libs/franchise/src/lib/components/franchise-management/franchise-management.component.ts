...
@Component({
    selector: 'easyroute-franchise-management',
    templateUrl: './franchise-management.component.html',
    styleUrls: ['./franchise-management.component.scss'],
})
export class FranchiseManagementComponent implements OnInit {
    ...
    cargar() {
        ...
        let url = environment.apiUrl + 'company_franchise_datatables';
        ...
        this.table = $(table).DataTable({
            ...
            columns: [
                {
                    data: 'name',
                    title: this._translate.instant('FRANCHISE.INTERNAL_NAME'),
                },
                {
                    data: 'nif',
                    title: this._translate.instant('COMPANIES.CIF'),
                },
                {
                    data: 'streetAddress',
                    title: this._translate.instant(
                        'FRANCHISE.FRANCHISE_MANAGEMENT.DIRECTION',
                    ),
                },
                {
                    data: 'scheduleStart',
                ...
                {
                    data: 'productReceptionTimeFromCompanyParent',
                    title: this._translate.instant('FRANCHISE.TIME_PRODUCTS_TO_FRANCHISE'),
                    render: ( data, type, row ) => {
                ...
                {
                    data: 'productReceptionEachTimeFromCompanyParent',
                    title: this._translate.instant('FRANCHISE.INTERVAL_PRODUCTS_TO_FRANCHISE'),
                    render: ( data, type, row ) => {
                ...
                {
                    data: 'receiveProductsFromCompanyParent',
                    title: this._translate.instant('FRANCHISE.PRODUCTS_TO_FRANCHISE'),
                    render: ( data, type, row ) => {
                        if ( data ) {
                            return (`
                                ...
                                return (
                                    '<div class="text-center">' +
                                        '<button class="btn btn-default isActive warning text-center green' +
                                            '" >' +
                                            this._translate.instant('GENERAL.TO_SHOW') +
                                        '</button> ' +
                                    '</div>'
                                );
                               /* return (`
                                ...
                                return (
                                    '<div class="text-center">' +
                                        '<button class="btn btn-default isActive warning text-center gray' +
                                            '">' +
                                            this._translate.instant('GENERAL.DO_NOT_SHOW') +
                                        '</button> ' +
                                    '</div>'
                                );
                                /* return (`
    ...
    editar(tbody: any, table: any, that = this) {
        $(tbody).on('click', 'div.editar', function() {
            ...
            that.Router.navigate(['franchise/store', data.id]);
        });
    }
