...
@Component({
  selector: 'easyroute-modal-add-riders',
  templateUrl: './modal-add-riders.component.html',
  styleUrls: ['./modal-add-riders.component.scss']
})
export class ModalAddRidersComponent implements OnInit {
  ...
  cargar(zone?: string) {
    let that = this;
    let url =  environment.apiUrl + 'users_rider_datatables';
    let tok = 'Bearer ' + this.authLocal.obtenerTokenLocalStorage();
    this.table = $('#riders').DataTable({
        destroy: true,
        serverSide: true,
        processing: false,
        stateSave: true,
        ...
        initComplete: function (settings, data) {
            ...
            $('.dataTables_empty').html('No hay datos disponibles en esta tabla, ' +
                '<a id="newStore" style="color: #0088cc; cursor:pointer; text-decoration: underline;">Añadir usuarios</a>');
            $('.dataTables_scrollBody thead tr').addClass('color-hidden-scroll');
        },
        drawCallback: (settings, json) => {
            setTimeout(() => {
                $('#riders').DataTable().columns.adjust();
            }, 1);
        },
        lengthMenu: [50, 100],
        dom: `
    ...
    $('#search-modal').on('keyup', function () {
        $('#riders').DataTable().search(this.value).draw();
    });

    $('.dataTables_filter').removeAttr("class");
    this.initEvents('#riders tbody', this.table);
    this.newStore('#riders tbody', this.table);
} 
  newStore(tbody: any, table: any, that = this) {
    $(tbody).on('click', '#newStore', function () {
        that.router.navigateByUrl('management/users/new/me/true');
