...
import { VehicleModalActiveStatusComponent } from '../vehicle-modal-active-status/vehicle-modal-active-status.component';
import { ModalCheckMaintenanceComponent } from './modal-check-maintenance/modal-check-maintenance.component';
@Component({
  selector: 'easyroute-vehicle-maintenance-form',
...
export class VehicleMaintenanceFormComponent implements OnInit {
  ...
  sliceString2(text: string) {

    if (text) {
      return sliceMediaString(text, 120, '(min-width: 960px)');
  ...
  updateStatus(){
    ...
    let data = {
      message:  this.translate.instant('VEHICLE_MAINTENANCE.ARE_YOU_SURE_TO_SEND_THIS_SURVEY_TO_THE_HISTORY')
    };

    let send = {
      maintenanceStatusId: 3
    }
    const modal = this._modalService.open(VehicleModalActiveStatusComponent, {
      backdrop: 'static',
  ...
  modal.result.then(
      (resp: boolean) => {
          if (resp) {
              ...
              this.loading.showLoading();
              this.stateEasyrouteService.updateStatusMaintenance(this.vehicleMaintenace.id, send).subscribe(
                ({ data }: any) => {
                  ...
                  this.loading.hideLoading();
                  this._router.navigate(['/vehicle-maintenance']);
                },
                (error) => {
                  this.loading.hideLoading();
  ...
  forceFinished () {
    const modal = this._modalService.open(ModalCheckMaintenanceComponent, {
      backdrop: 'static',
    ...
    modal.componentInstance.title = this.translate.instant('VEHICLE_MAINTENANCE.CHECK_MAINTENANCE');
    modal.componentInstance.subTitle = this.translate.instant('VEHICLE_MAINTENANCE.ARE_YOU_SURE_YOU_WANT_TO_VERIFY');
    modal.componentInstance.message = this.translate.instant('VEHICLE_MAINTENANCE.TEXT_5');
    modal.componentInstance.titleBotton = this.translate.instant('VEHICLE_MAINTENANCE.YES_CHECK');
