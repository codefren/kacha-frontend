...
declare var $;
@Component({
  selector: 'easyroute-modal-delivery-point-pending',
  templateUrl: './modal-delivery-point-pending.component.html',
  styleUrls: ['./modal-delivery-point-pending.component.scss']
})
export class ModalDeliveryPointPendingComponent implements OnInit {
  ...
  cargar() {
    ...
    this.selected = [];
    let url = environment.apiUrl + 'route_planning/delivery_point_pending_datatables';
      ...
      columns: [
        ...
        },
        {
          ...
          render: (data, type, row) => {
            ...
            return (`
                        <div class="justify-content-center delete">
                          <div class="col-12">
                           <img src="assets/icons/delete-pending.svg">
                          </div>
                        </div>
                      `);
          }
    ...
    $('.optimroute-integration-table').find('.label-search').html(`
        ...
        <div class="col-md-12 col-12 p-0">
            <div class="d-flex flex-lg-row flex-column justify-content-end align-items-center align-items-lg-start">
  ...
  delete(tbody: any, table: any, that = this){
    $(tbody).on('click', '.delete', async function () {
      ...
      await that.loading.showLoading();
      that.backend.delete('route_planning/delivery_point_pending/' + data.id).pipe(take(1)).subscribe(()=>{
        that.table.ajax.reload();
        that.loading.hideLoading();
        that.facade.getDeliveryPointPending();
      });
    });
  }
