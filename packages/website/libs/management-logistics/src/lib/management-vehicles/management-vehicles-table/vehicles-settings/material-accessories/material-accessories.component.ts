...
import { ModalAddMaterialAccessoriesComponent } from './modal-add-material-accessories/modal-add-material-accessories.component';
@Component({
  selector: 'easyroute-material-accessories',
  templateUrl: './material-accessories.component.html',
  styleUrls: ['./material-accessories.component.scss']
})
export class MaterialAccessoriesComponent implements OnInit {
  ...
  cargar() {
    ...
    let url = environment.apiUrl + 'company_material_complement_datatables';
    ...
    this.table = $(table).DataTable({
        ...
        columns: [
           ...
           /*  {
                data: 'name',
                className: 'text-aling-table',
                title: this.translate.instant('VEHICLES.SERVICE_TYPE.SPECIFICATION'),
    ...
    this.initEvents('#material tbody', this.table);
}
  ...
  load(){
    ...
    this.backendService.get('company_material_complement').pipe(take(1)).subscribe((data)=>{
...
updateIsActive(Id: number, element: any , vehicle: any) {
  ...
  this.backendService.put('company_material_complement/'+Id,{isActive:element}).pipe(take(1)).subscribe((data)=>{
  ...
  modalAddOtherLicense(){
    ...
    const modal = this._modalService.open(ModalAddMaterialAccessoriesComponent, {
...
//          this.load();
  ...
  deleteOtherLicense(data:any){
    ...
    modal.componentInstance.message = this.translate.instant('VEHICLES.SETTINGS.MESSAGE_DELETE_MATERIAL_ACCESORIES');
