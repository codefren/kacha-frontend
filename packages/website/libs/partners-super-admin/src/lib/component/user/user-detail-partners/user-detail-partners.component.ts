...
import { UserMessages, UtilData, ValidateCompanyId, ValidatePhone, dayTimeAsStringToSeconds, secondsToDayTimeAsString } from '@optimroute/shared';
...
declare var $: any;
@Component({
  selector: 'easyroute-user-detail-partners',
  templateUrl: './user-detail-partners.component.html',
  styleUrls: ['./user-detail-partners.component.scss']
})
export class UserDetailPartnersComponent implements OnInit {
...
initForm(user: User) {
    ...
    this.createUserFormGroup = this.fb.group({
        ...
        userTypeId:[{ value: user.userType !=null ? user.userType.id :'', disabled: true }]
    });
...
cargarDocument() {
  ...
  this.initEvents('#userDetailDocument tbody', this.tableDocument);
}
