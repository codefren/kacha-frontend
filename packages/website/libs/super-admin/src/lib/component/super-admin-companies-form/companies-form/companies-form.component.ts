...
import { Component, OnInit, ChangeDetectorRef, Input, Output, EventEmitter, OnDestroy, NgZone } from '@angular/core';
import { Company } from '../../../../../../backend/src/lib/types/companies.type';
import { NgbDateStruct, NgbModal } from '@ng-bootstrap/ng-bootstrap';
import { ModuleInterface } from '../../../../../../backend/src/lib/types/module.type';
...
import { DemoDialogComponent } from '../../super-admin-companies-list/demo-dialog/demo-dialog.component';
import { ModalCheckCostComponent, dateToObject, getToday, objectToString } from '@optimroute/shared';
...
export class CompaniesFormComponent implements OnInit, OnDestroy {
   ...
   /*  @Output('companies')
    ...
    companies =  new EventEmitter<Company>();
    countries: string[] = [];
    ...
    async initForm(dataCompany: Company, id: any) {
        ...
        this.imageName = dataCompany.nameLogoTypePartner;
        ...
        this.createCompanyFormGroup = this.fb.group({
          ...
          /*   phone: [
            ...
            phones:[],
            isPartnerType:[dataCompany.isPartnerType],

            hideMultidelegation: [dataCompany.hideMultidelegation],

            activeLogoNomenclature:[dataCompany.activeLogoNomenclature],
            urlPartnerType:[
                dataCompany.urlPartnerType,
            ],
            logotypePartner:[dataCompany.urlLogoTypePartner],
            integrationTxt: [dataCompany.integrationTxt ? dataCompany.integrationTxt : false]
    ...
    addCompany(company: Company) {
        ...
        this.companiesService.addCompany(company).subscribe(
            (data: any) => {
                ...
                this.ngZone.run(() => this.router.navigate(['/super-admin/company/', data.data.id]).then());
        ...
        this.companiesFacade.updated$
            ...
            .subscribe((data) => {
                if (data) {
                    ...
                    this.ngZone.run(() => this.router.navigate(['/super-admin/company']).then());
                    /*  if (this.me) {
                         this.router.navigate(['/users-management/companiesMe/']);
                     } else {
                         this.router.navigate(['/users-management/companies/']);
    ...
    changeParnetSelecter(event:any){

        let data = {
            ...this.company,
            isPartnerType: event.target.checked
    ...
    changeActiveLogoNomenclature(event:any){

        let data = {
            ...this.company,
            activeLogoNomenclature: event.target.checked

        };

        if (event.target.checked) {
            this.createCompanyFormGroup.controls['urlPartnerType'].setValidators(
                [
                    Validators.required,
            ...
            this.createCompanyFormGroup.controls['logotypePartner'].setValidators(
               [ Validators.required]
            );

        } else {

            this.createCompanyFormGroup.get('urlPartnerType').setValidators(null);
            this.createCompanyFormGroup.get('logotypePartner').setValidators(null);

        }

        this.createCompanyFormGroup.get('logotypePartner').updateValueAndValidity();
        this.createCompanyFormGroup.get('urlPartnerType').updateValueAndValidity();
    ...
    validateSizeImg( $event) {
        ...
        this.createCompanyFormGroup.get('logotypePartner').setValue( reader );
    ...
    openDelete(){
        ...
        const modal = this.dialog.open(ModalCheckCostComponent, {
        ...
        modal.componentInstance.title = this.translate.instant('GENERAL.DELETE');

        modal.componentInstance.message = this.translate.instant('COMPANIES.DELETE_MESSAGE_COMPANY') + this.company.name +'?';

        modal.componentInstance.accept =  this.translate.instant('GENERAL.YES_DELETE');
        ...
        modal.result.then((data) => {
            ...
            if (data) {
                ...
                this.backendService.delete('company_delete/'+this.company.id)
                .pipe(take(1))
                .subscribe((response) => {
                    ...
                    this.toastService.displayWebsiteRelatedToast(
                        this.translate.instant('GENERAL.DELETE_SUCCESSFUL'),
                        this.translate.instant('GENERAL.ACCEPT'),
                    );
