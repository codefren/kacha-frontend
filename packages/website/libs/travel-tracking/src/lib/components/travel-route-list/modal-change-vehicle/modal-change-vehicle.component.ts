...
declare var $: any;
@Component({
  selector: 'easyroute-modal-change-vehicle',
  templateUrl: './modal-change-vehicle.component.html',
  styleUrls: ['./modal-change-vehicle.component.scss']
})
export class ModalChangeVehicleComponent implements OnInit {
    ...
    this.table = $('#vehicle').DataTable({
      ...
      columns: [
        ...
        },
        {
          ...
          render: (data, type, row) => {
              let nameDriver = row['user'] ? row['user']['name'] + ' ' +  row['user']['surname'] : '';
              return (
                  '<span data-toggle="tooltip" data-placement="top" title="' +
                  data +
                  '">' +
  ...
  selectVehicle(tbody: any, table: any, that = this){
    ...
    $(tbody).on('click', 'tr', function () {
      ...
      if (index === -1) {
        ...
        that.idVehicleSelected = data.id;

        that.nameVehicleSelected = data.name;
        ...
        that.idVehicleSelected = undefined;

        that.nameVehicleSelected = undefined;
