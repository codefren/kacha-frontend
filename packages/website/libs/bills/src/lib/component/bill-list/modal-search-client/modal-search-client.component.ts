...
import { ClientInterface } from 'libs/backend/src/lib/types/clients.type';
declare var $: any;
@Component({
  selector: 'easyroute-modal-search-client',
...
export class ModalSearchClientComponent implements OnInit {
  ...
  cargar() {
    ...
    let table = '#clientsBills';
    this.tableClient = $(table).DataTable({
      destroy: true,
      ...
      initComplete : function (settings, data) {
          ...
          $('#clientsBills .dataTables_empty').html('No hay datos disponibles en esta tabla, '+
      ...
      drawCallback: (settings, json) =>{
        setTimeout(() => {
            $('#clientsBills').DataTable().columns.adjust();
        }, 1);
      },
      lengthMenu: [50, 100],
      dom: `
    ...
    this.editar('#clientsBills tbody', this.tableClient);
    this.newClient('#clientsBills tbody', this.tableClient);
  }
